...
from .utils import generate_csv_report, normalize_data, reconcile
...
class CSVReconciliationView(View):
    ...
    def post(self, request) -> JsonResponse:
        ...
        if not source_file or not target_file:
            return JsonResponse(
                {"error": "Both source and target files are required."}, status=400
            )

        try:
            # Save uploaded files temporarily
            source_path = default_storage.save(
                "source.csv", ContentFile(source_file.read())
            )
            target_path = default_storage.save(
                "target.csv", ContentFile(target_file.read())
            )
            ...
            # Reconcile records
            missing_in_target, missing_in_source, discrepancies = reconcile(
                source_df, target_df
            )
            ...
            if report_format == "csv":
                response = generate_csv_report(
                    missing_in_target, missing_in_source, discrepancies
                )
                return response
